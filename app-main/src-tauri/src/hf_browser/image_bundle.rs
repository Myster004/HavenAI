...
use crate::image_generator::sdcpp::{self, HfBundleRegistration, RemoteBundleRunnabilityRequest};
...
fn repo_compatibility(profile: &sdcpp::HfBundleProfile, detail: &ApiModel) -> (bool, String) {
    ...
    if explicitly_wrong_variant {
        return (
            false,
            "The repository belongs to a different variant of this model family.".to_string(),
        );
    }
    if profile
        .diffusion_markers
        ...
        .any(|marker| evidence.iter().any(|value| value.contains(marker)))
    {
        return (
            true,
            "Architecture or declared base-model ancestry matches this recipe.".to_string(),
        );
    }
    (
        false,
...
fn role_default_query(profile: &sdcpp::HfBundleProfile, role: &str) -> String {
    match role {
        "diffusion_model" => profile
            .diffusion_markers
            .first()
            .copied()
            .unwrap_or_default()
            .to_string(),
        "text_encoder" | "vision_encoder" => profile
            .encoder_markers
            .first()
            .copied()
            .unwrap_or_default()
...
pub(crate) async fn bundle_role_search(
    ...
    let mut base =
        format!("https://huggingface.co/api/models?limit=100&sort={sort_field}&direction=-1");
    ...
    let mut results =
        super::fetch_models_list(app, &format!("{base}&filter={format_filter}")).await?;
...
fn role_compatible(
    ...
    match role {
        ...
        "vae" if profile.id.starts_with("krea") => {
            evidence.contains("wan") && evidence.contains("vae")
        }
        "vae" if profile.id == "qwen-image-edit-2511" => {
...
pub async fn hf_image_bundle_files(
    ...
    let model_id = parse_model_id(&model_id)
        .ok_or_else(|| "Enter a valid Hugging Face model ID or URL.".to_string())?;
    let client = authenticated_client(&app)?;
    let detail = fetch_detail(&client, &model_id).await?;
    let revision = detail
        .sha
    ...
    for entry in tree {
        if entry.entry_type != "file"
            ...
            || excluded_artifact(&entry.path)
        {
            continue;
        }
        let Some(format) = format_of(&entry.path) else {
            continue;
        };
        ...
        prevalidated.into_iter().zip(architectures)
    {
        ...
        VALIDATED_ASSETS
            .lock()
            .await
            .insert(asset.selection_id.clone(), asset.clone());
...
fn write_manifest(app: &AppHandle, manifest: &BundleManifest) -> Result<(), String> {
    ...
    let parent = path
        .parent()
        .ok_or_else(|| "Invalid bundle manifest path.".to_string())?;
...
async fn refetch_asset(app: &AppHandle, asset: &ValidatedAsset) -> Result<ValidatedAsset, String> {
    ...
    let entry = tree
        ...
        .ok_or_else(|| {
            "A selected component no longer exists at the pinned revision.".to_string()
        })?;
    let size = entry.lfs.as_ref().map(|lfs| lfs.size).unwrap_or(entry.size);
    let sha256 = entry.lfs.and_then(|lfs| lfs.oid).unwrap_or_default();
    if size != asset.size || sha256 != asset.sha256 {
        return Err(
            "A selected component no longer matches its validated size and hash.".to_string(),
        );
    }
    Ok(asset.clone())
}
...
fn asset_destination(app: &AppHandle, asset: &ValidatedAsset) -> Result<PathBuf, String> {
    ...
    let author = parts
        .next()
        .ok_or_else(|| "Invalid Hugging Face author.".to_string())?;
    let repository = parts
        .next()
        .ok_or_else(|| "Invalid Hugging Face repository.".to_string())?;
...
pub async fn hf_queue_image_bundle(
    ...
    for selection_id in &request.selection_ids {
        let asset = selected
            ...
            .cloned()
            .ok_or_else(|| {
                "A component selection expired. Select it again before downloading.".to_string()
            })?;
        assets.push(asset);
    }
    drop(selected);
    ...
    .await?;
    if matches!(
        backend_estimate.status.as_str(),
        "notInstalled" | "incompatibleRuntime"
    ) {
    ...
    for asset in assets {
            ...
            && !role_compatible(&profile, &asset.role, &asset.model_id, &asset.relative_path)
        {
            return Err(format!(
                ...
                asset.role, profile.display_name
            ));
        }
    ...
    let mut manifest = BundleManifest {
        ...
        runtime_asset: request.runtime_asset.clone(),
        runnability: Some(
            serde_json::to_value(&backend_estimate)
                .map_err(|error| format!("Failed to save the runnability estimate: {error}"))?,
        ),
        assets: Vec::new(),
    ...
    for asset in &refreshed {
        manifest.assets.push(ManifestAsset {
            local_path: asset_destination(&app, asset)?
                .to_string_lossy()
                .to_string(),
            asset: asset.clone(),
            verified: false,
    ...
    for asset in &refreshed {
        let destination = asset_destination(&app, asset)?
            .to_string_lossy()
            .to_string();
...
fn register_manifest(app: &AppHandle, manifest: &mut BundleManifest) -> Result<String, String> {
    ...
    sdcpp::register_hf_bundle_model(
        ...
        HfBundleRegistration {
            ...
            diffusion_path: by_role
                .get("diffusion_model")
                .copied()
                .ok_or_else(|| "The diffusion model is missing.".to_string())?,
            text_encoder_path: by_role
                .get("text_encoder")
                .copied()
                .ok_or_else(|| "The text encoder is missing.".to_string())?,
            vae_path: by_role
                .get("vae")
                .copied()
                .ok_or_else(|| "The VAE is missing.".to_string())?,
...
pub(super) async fn handle_download_completed(
    ...
    let Some(bundle_id) = item.install_id.as_deref() else {
        return;
    };
    let _guard = MANIFEST_LOCK.lock().await;
    let Ok(mut manifest) = read_manifest(app, bundle_id) else {
        return;
    };
...
mod tests {
    ...
    fn qwen_edit_recipe_requires_a_vision_encoder() {
        ...
        assert!(!hf_bundle_profile("z-image")
            .unwrap()
            .required_roles
            .contains(&"vision_encoder"));
    }
}
