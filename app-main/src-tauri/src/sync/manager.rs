...
use crate::sync::v2::protocol::{
    validate_hello, BlobDescriptor, BlobRequestDescriptor, SyncHello, SyncLimits, SyncV2Message,
    SYNC_V2_PROTOCOL_VERSION,
};
use crate::sync::v2::{
    apply_staged_batch, begin_blob_receive, build_outbound_batch, cached_schema_fingerprint,
    finish_blob_receive, get_or_create_device_id, load_frontier, plan_outbound,
    record_peer_acknowledgement, revision_batch_hash, stage_revision_batch, write_blob_chunk,
    BatchApplyResult, BlobReceiveState, ChangeRevision,
};
...
async fn stage_and_apply_batch_with_retry(
    ...
    loop {
        ...
        match result {
            ...
            Err(error)
                if error.is_retryable_lock() && retry < DATABASE_LOCK_RETRY_DELAYS_MS.len() =>
            {
...
impl TransferStats {
    ...
    fn database_wait_status(&self, attempt: u64, total: u64, delay_ms: u64) -> SyncStatus {
...
pub struct SyncManagerState {
    ...
    pub pending_starts: RwLock<HashMap<String, tokio::sync::oneshot::Sender<()>>>,
...
async fn set_sharing_status(app: &AppHandle, state: &SyncManagerState, port: u16) {
...
pub async fn connect_device(
    ...
    state.set_status(&app, SyncStatus::Connecting).await;
    ...
    *shutdown = Some(shutdown_tx);
    ...
    let app_clone = app.clone();
    tokio::spawn(async move {
        let result = run_passenger_session(app_clone.clone(), stream, &mut shutdown_rx, pin).await;
        let state = app_clone.state::<SyncManagerState>();
        let owns_session = {
            let mut current = state.shutdown_tx.lock().await;
            match current.as_ref() {
        ...
        match result {
            Ok(()) => {
                state.set_status(&app_clone, SyncStatus::Completed).await;
            }
            Err(error) if error == "Sync cancelled" => {
                state.set_status(&app_clone, SyncStatus::Idle).await;
            }
...
async fn run_passenger_session(
    ...
    app.state::<SyncManagerState>()
        .set_status(&app, SyncStatus::WaitingForApproval { peer: peer_name })
        .await;
    match receive_message(&mut framed).await? {
        P2PMessage::Ready => {}
...
async fn run_v2_replication(
    ...
    framed
        .send(P2PMessage::Sync(SyncV2Message::Hello(local_hello.clone())))
        .await
        .map_err(|error| error.to_string())?;
    ...
    framed
        .send(P2PMessage::Sync(SyncV2Message::Frontier(local_frontier)))
        .await
        .map_err(|error| error.to_string())?;
    ...
    framed
        .send(P2PMessage::Sync(SyncV2Message::Plan(outbound_plan.clone())))
        .await
        .map_err(|error| error.to_string())?;
    ...
    loop {
        ...
        let outgoing = if let Some(batch_id) = &sent_batch_id {
            P2PMessage::Sync(SyncV2Message::RevisionBatch {
                ...
                batch_hash: revision_batch_hash(&revisions).map_err(|error| error.to_string())?,
        ...
        let (received_batch_id, received_quiescent) = match incoming {
            ...
            P2PMessage::Sync(SyncV2Message::Quiescent { .. }) => (QUIESCENT_ACK.to_string(), true),
...
async fn sync_assets(
    ...
    let state = app.state::<SyncManagerState>();
    state.set_status(app, stats.status("Comparing files")).await;
    ...
    for mut descriptor in peer_inventory {
        ...
        crate::sync::v2::assets::retain_remote_winners(&mut descriptor, &local_path_hashes);
        ...
        let already_materialized =
            descriptor
                .relative_paths
                .iter()
                .try_fold(true, |all_present, path| {
        ...
        match receive_state {
            ...
            BlobReceiveState::Ready { offset } => {
                ...
                requested_descriptors.insert(descriptor.content_hash.clone(), descriptor);
            }
        }
    }
    ...
    stats.planned_bytes = stats.planned_bytes.saturating_add(
        peer_requests
            ...
            .filter_map(|request| {
                local_assets
                    .get(&request.content_hash)
                    .map(|asset| asset.descriptor.size_bytes.saturating_sub(request.offset))
            })
            .sum::<u64>(),
    );
    state
        ...
        loop {
            match receive_split(stream, stop).await? {
                P2PMessage::Sync(SyncV2Message::BlobChunk {
                    ...
                    expected_offset = {
                        let conn = crate::storage_manager::db::open_db(app)?;
                        write_blob_chunk(&conn, cache_root, &content_hash, offset, &bytes)
                            .map_err(|error| error.to_string())?
                    };
                    stats.bytes_received = stats.bytes_received.saturating_add(bytes.len() as u64);
                    app.state::<SyncManagerState>()
                        .set_status(app, stats.status("Exchanging files"))
                        .await;
                }
                P2PMessage::Sync(SyncV2Message::BlobComplete {
                    ...
                    sink.send(P2PMessage::Sync(SyncV2Message::BlobAck { content_hash }))
                        .await
                        .map_err(|error| error.to_string())?;
...
fn local_hello(app: &AppHandle) -> Result<SyncHello, String> {
    let conn = crate::storage_manager::db::open_db(app)?;
    crate::sync::v2::ensure_current_database_seeded(&conn, crate::utils::now_millis()? as i64)
        .map_err(|error| error.to_string())?;
    Ok(SyncHello {
        app_version: app.package_info().version.to_string(),
        sync_protocol_version: SYNC_V2_PROTOCOL_VERSION,
        schema_fingerprint: cached_schema_fingerprint(&conn).map_err(|error| error.to_string())?,
...
async fn receive_message(framed: &mut Framed<TcpStream, P2PCodec>) -> Result<P2PMessage, String> {
...
fn encrypt_challenge(cipher: &ChaCha20Poly1305, challenge: &[u8; 16]) -> Result<Vec<u8>, String> {
