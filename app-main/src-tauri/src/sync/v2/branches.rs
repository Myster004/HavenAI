...
fn materialize_branch(
    ...
    let subtree = descendant_ids(tx, config.message_table, source_session_id, losing_child_id)?;
    let message_table = quote_identifier(config.message_table);
...
fn clone_message_variants(
    ...
    for variant_id in variant_ids {
        ...
        overrides.insert("message_id", Value::Text(cloned_message_id.to_string()));
...
mod tests {
    ...
    fn divergent_tails_become_deterministic_separate_sessions() {
        ...
        let branch_id: String = conn
            ...
            .unwrap();
        let branch = conn
            .prepare("SELECT content FROM messages WHERE session_id = ?1 ORDER BY created_at, id")
            .unwrap()
        ...
        assert_eq!(
            materialize_message_forks(&conn.unchecked_transaction().unwrap()).unwrap(),
            0
        );
    }
