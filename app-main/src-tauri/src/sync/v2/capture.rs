...
pub fn ensure_current_database_seeded(
    ...
    for table in tables {
        ...
        let table_fingerprint = blake3::hash(table.create_sql.as_bytes())
            .to_hex()
            .to_string();
...
mod tests {
    ...
    use super::{
        ...
        ensure_current_database_seeded,
    };
    ...
    fn user_write_and_outbox_revision_commit_atomically() {
        ...
        assert_eq!(
            ...
            blake3::hash(&revision.changeset).to_hex().to_string()
        );
        assert_eq!(
    ...
    fn app_capture_reuses_the_v2_device_identity() {
        ...
        assert_eq!(captured.revision.unwrap().origin_device_id, "stable-device");
    }
    ...
    fn existing_rows_are_seeded_once_for_v2_bootstrap() {
        ...
        let revision = source
            .query_row("SELECT change_id FROM sync_v2_changes LIMIT 1", [], |row| {
                row.get::<_, String>(0)
            })
            .unwrap();
        ...
        assert_eq!(
            target
                .query_row("SELECT title FROM chats WHERE id = 'chat-1'", [], |row| row
                    .get::<_, String>(0),)
                .unwrap(),
            "Existing"
        );
    ...
    fn bootstrap_chunks_foreign_key_tables_without_requiring_the_parent_in_scratch() {
        ...
        assert_eq!(
            target
                .query_row(
                    "SELECT note FROM character_notes WHERE id = 'note-1'",
                    [],
                    |row| { row.get::<_, String>(0) }
                )
                .unwrap(),
