...
pub enum ApplyError {
    ...
    #[error("revision {change_id} uses schema {received}, but this device uses {expected}")]
    SchemaMismatch {
        change_id: String,
        expected: String,
        received: String,
...
pub fn apply_remote_revision(
    ...
    for row in changes {
        ...
        if is_concurrent {
            ...
            let incoming_row =
                materialize_incoming_snapshot(&tx, &row, &current_row, &current_revision, revision)
                    .ok();
    ...
    advance_frontier(&tx, &revision.origin_device_id, revision.origin_sequence)?;
...
fn load_row_revision(
    ...
    let revision = load_revision(tx, &change_id)?.ok_or_else(|| ApplyError::DuplicateMismatch {
...
fn load_row_snapshot(tx: &Transaction<'_>, row: RowChange) -> Result<RowSnapshot, rusqlite::Error> {
...
fn restore_snapshot(tx: &Transaction<'_>, snapshot: &RowSnapshot) -> Result<(), rusqlite::Error> {
    ...
    if let Some(values) = &snapshot.values {
        ...
        tx.execute(&sql, params_from_iter(snapshot.row.primary_key.iter()))?;
    }
    Ok(())
}
...
fn materialize_incoming_snapshot(
    ...
    apply_snapshot_changeset(&scratch, &row.table_name, &incoming_revision.changeset)?;
...
fn insert_conflict(
    ...
    let identity = format!(
        "{}\0{}\0{}\0{}",
        conflict.row.table_name, conflict.row.primary_key_hash, revision_ids[0], revision_ids[1]
    );
...
mod tests {
    ...
    fn initialize_connection(conn: &Connection) {
        ...
        .unwrap();
        conn.execute("INSERT INTO local_usage (id, elapsed_ms) VALUES (1, 0)", [])
            .unwrap();
    ...
    fn rejects_missing_predecessors_without_advancing_frontier() {
        ...
        let first = capture_transaction(&source, "device-a", 100, |tx| {
            tx.execute("INSERT INTO chats (id, title) VALUES ('chat-1', 'one')", [])
        })
        .unwrap()
        .revision
        .unwrap();
