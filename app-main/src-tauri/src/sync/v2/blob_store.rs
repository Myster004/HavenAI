...
pub fn write_blob_chunk(
    ...
    let partial_path = partial_path(root, content_hash);
    let actual_offset = partial_path
        .metadata()
...
fn quarantine_partial(
    ...
    if partial_path.exists() {
        let quarantine = root
            .join(".incoming")
            .join(format!("{content_hash}.corrupt-{}", Uuid::new_v4()));
...
mod tests {
    ...
    fn blob_download_resumes_verifies_and_streams() {
        ...
        assert_eq!(
            begin_blob_receive(&conn, root.path(), &hash, content.len() as u64, 100).unwrap(),
        ...
        assert_eq!(
            begin_blob_receive(&conn, root.path(), &hash, content.len() as u64, 110).unwrap(),
