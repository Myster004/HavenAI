...
pub fn list_unresolved_conflicts(conn: &Connection) -> Result<Vec<SyncConflict>, ConflictError> {
    ...
    for row in rows {
        ...
        let Some(winning_change_id) = winning_change_id(conn, &table_name, &local.primary_key)?
        else {
            continue;
        };
        let (
...
mod tests {
    ...
    fn user_choice_is_captured_and_resolves_on_every_replica() {
        ...
        resolve_conflict(&left, &conflicts[0].conflict_id, ConflictChoice::Other, 400).unwrap();
