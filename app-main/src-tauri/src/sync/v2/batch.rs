...
pub fn stage_revision_batch(
    ...
    if let Some((existing_hash, expected_revisions)) = conn
        ...
        .optional()?
    {
        if existing_hash != declared_batch_hash || expected_revisions != revisions.len() as i64 {
...
fn revision_is_ready(conn: &Connection, revision: &ChangeRevision) -> Result<bool, BatchError> {
    ...
    Ok(revision
        .base_frontier
        .iter()
        .all(|(device_id, required)| frontier.get(device_id).copied().unwrap_or(0) >= *required))
}
...
mod tests {
    ...
    fn durable_batch_applies_in_order_and_replay_is_safe() {
        ...
        assert_eq!(
            stage_revision_batch(&target, "device-a", "batch-1", &hash, &revisions, 210).unwrap(),
            StageOutcome::Staged
        );
        ...
        assert_eq!(
            stage_revision_batch(&target, "device-a", "batch-1", &hash, &revisions, 230).unwrap(),
            StageOutcome::Duplicate
        );
        assert!(
            apply_staged_batch(&target, "batch-1", 240)
                .unwrap()
                .already_committed
        );
    }
    ...
    fn batch_applies_revisions_in_dependency_order() {
        ...
        stage_revision_batch(&target, "device-a", "batch-1", &hash, &reversed, 210).unwrap();
    ...
    fn invalid_batch_hash_is_rejected_before_staging() {
        ...
        assert!(matches!(
            stage_revision_batch(&target, "device-a", "batch-1", "invalid", &[revision], 110),
