...
pub fn collect_local_assets(
    ...
    for directory in ["avatars", "images", "sessions"] {
        collect_directory(&storage_root.join(directory), directory, &mut by_hash)?;
    }
    collect_directory(
...
pub fn destination_path(app: &tauri::AppHandle, relative_path: &str) -> Result<PathBuf, String> {
...
mod tests {
    ...
    fn concurrent_file_edits_choose_the_same_content_on_both_devices() {
        ...
        retain_remote_winners(&mut low_remote, &BTreeMap::from([(path, high_hash)]));
