...
pub fn build_outbound_batch(
        ...
        for sequence in range.first_sequence..=range.last_sequence {
            let change_id = conn
                ...
                .map_err(|error| match error {
                    rusqlite::Error::QueryReturnedNoRows => ReplicationError::MissingRevision {
                        origin_device_id: range.origin_device_id.clone(),
    ...
    while !pending.is_empty() && batch.len() < max_revisions {
        ...
        simulated_frontier.insert(revision.origin_device_id.clone(), revision.origin_sequence);
        batch.push(revision);
    }
    Ok(batch)
}
...
pub fn record_peer_acknowledgement(
    ...
    for (origin_device_id, acknowledged_sequence) in frontier {
        ...
        tx.execute(
            ...
            params![peer_device_id, origin_device_id, bounded_sequence, now_ms],
        )?;
    }
    tx.commit()?;
    Ok(())
...
fn is_ready(revision: &ChangeRevision, frontier: &BTreeMap<String, i64>) -> bool {
    frontier
        ...
        && revision
            .base_frontier
            .iter()
            .all(|(device_id, sequence)| frontier.get(device_id).copied().unwrap_or(0) >= *sequence)
}
...
mod tests {
    ...
    use super::{build_outbound_batch, plan_outbound, record_peer_acknowledgement};
    ...
    fn outbound_batch_respects_cross_device_causal_dependencies() {
        ...
        let batch = build_outbound_batch(&relay, &BTreeMap::new(), 128, 1024 * 1024).unwrap();
    ...
    fn plan_and_acknowledgement_use_monotonic_frontiers() {
        ...
        let plan = plan_outbound(&conn, &BTreeMap::from([("device-a".to_string(), 1)])).unwrap();
