...
fn set_local_integer(tx: &Transaction<'_>, key: &str, value: i64) -> Result<(), rusqlite::Error> {
    tx.execute(
...
fn resolve_dominated_conflicts(
    ...
    let rows = statement.query_map(params![row.table_name, row.primary_key_bytes], |result| {
        Ok((
