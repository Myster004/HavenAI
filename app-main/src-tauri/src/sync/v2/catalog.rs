...
pub fn syncable_tables(conn: &Connection) -> Result<Vec<TableInfo>, CatalogError> {
    ...
    for row in rows {
        ...
        let mut pk_statement = conn.prepare(&format!(
            "SELECT name, pk FROM pragma_table_info('{escaped_name}')"
        ))?;
...
pub fn schema_fingerprint(conn: &Connection) -> Result<String, CatalogError> {
    ...
    for table in tables {
        ...
        for column in columns {
            let (cid, name, column_type, not_null, default_value, primary_key, hidden) = column?;
...
mod tests {
    ...
    use super::{
        audit_sync_catalog, cached_schema_fingerprint, is_syncable_table, schema_fingerprint,
    };
