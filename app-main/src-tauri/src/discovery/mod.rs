...
pub(crate) fn read_pure_mode_level(app: &AppHandle) -> String {
...
pub(crate) fn is_dynamic_memory_enabled(app: &AppHandle) -> bool {
    if let Ok(Some(raw)) = internal_read_settings(app) {
        if let Ok(json) = serde_json::from_str::<Value>(&raw) {
            return json
...
pub(crate) fn cache_set<T: Serialize>(key: String, value: &T, ttl_secs: i64) {
...
pub(crate) fn push_definition_block(
