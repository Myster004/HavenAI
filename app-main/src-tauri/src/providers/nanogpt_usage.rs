...
fn primary_window(usage: &NanoGptSubscriptionUsage) -> Option<(&'static str, &NanoGptQuotaWindow)> {
...
fn window_percent(window: &NanoGptQuotaWindow) -> Option<f64> {
    match (window.used, window.limit) {
        ...
        _ => window.percent_used.map(|percent| percent.clamp(0.0, 1.0)),
    }
}
...
fn pending_warning(
    ...
    if warned
        .get(&usage.credential_id)
        .is_some_and(|record| record.window == window_key && record.threshold >= threshold)
    {
        return None;
    }
    warned.insert(
...
pub async fn fetch_subscription_usage(
    ...
    if !status.is_success() {
        ...
        return Err(match provider_message {
            ...
            None => format!(
                "NanoGPT returned {} while fetching subscription usage.",
                status
            ),
        });
    }
