...
pub const FALLBACK_CONTEXT_LENGTH: u32 = 8192;
...
pub(super) fn resolve_context_length(
    ...
    if let Some(v) = explicit {
        ...
        // No explicit configuration - will be resolved asynchronously with provider metadata
        // Return None here to signal that async resolution is needed
        None
    }
}

/// Resolve context length asynchronously using provider metadata when not explicitly configured
pub async fn resolve_context_length_async(
    ...
    if resolution.source == crate::chat_manager::model_metadata::ContextLengthSource::Fallback {
        eprintln!(
