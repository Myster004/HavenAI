...
fn append_soul_growth_gated(
    ...
    for mut entry in entries {
        ...
        if entry
            .valid_until
            .is_some_and(|until| until <= entry.valid_from)
        {
            continue;
        }
...
pub fn set_soul_growth_lock(session: &mut Session, entry_id: &str, locked: bool, now: u64) -> bool {
    let raw = match &session.companion_state {
        Some(raw) => raw.clone(),
        None => return false,
    };
...
mod tests {
    ...
    fn turn_effect_seed_excludes_passive_state_drift() {
        ...
        assert_eq!(seed.signal_changes["removed"], json!(["emotion:positive"]));
    }
