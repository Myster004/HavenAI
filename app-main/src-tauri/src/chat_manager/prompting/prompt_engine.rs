...
pub fn resolve_used_lorebook_entries(
    ...
    let lorebook_ids = match session.lorebook_ids_override.as_ref() {
        Some(lorebook_ids_override) => lorebook_ids_override.clone(),
        None => {
    ...
    let active_entries = match crate::chat_manager::prompting::lorebook_matcher::select_lorebook_entries_with_budget_for_context(
        app,
        &conn,
        &lorebook_ids,
        &recent_messages,
...
pub fn build_system_prompt_entries(
    ...
    let avatar_generation_enabled = settings
        ...
        .unwrap_or(true);
    let is_scene_generation_local_image_model = scene_generation_uses_local_image_model(settings);
    let (character_lora_keywords, persona_lora_keywords) = if is_scene_generation_local_image_model
    {
    ...
    for entry in base_entries.iter() {
        ...
        .replace("{{lora_keywords[character]}}", &character_lora_keywords)
...
mod prompt_cache_tests {
    use super::{
        ...
        default_companion_entries, default_group_chat_entries, default_group_chat_roleplay_entries,
        default_local_roleplay_entries, default_modular_prompt_entries,
        default_scene_prompt_writer_entries, entry_contains_volatile_turn_context,
    };
    ...
    fn has_scene_local_image_model_gate(condition: &PromptEntryCondition, expected: bool) -> bool {
        match condition {
            PromptEntryCondition::IsSceneGenerationLocalImageModel { value } => *value == expected,
            PromptEntryCondition::All { conditions } | PromptEntryCondition::Any { conditions } => {
                conditions
                    .iter()
    ...
    fn has_direct_chat_gate(condition: &PromptEntryCondition) -> bool {
        match condition {
            PromptEntryCondition::ChatMode { value } => {
                matches!(
                    value,
                    crate::chat_manager::types::PromptEntryChatMode::Direct
                )
            }
            PromptEntryCondition::All { conditions } | PromptEntryCondition::Any { conditions } => {
    ...
    fn app_default_scene_protocol_exposes_lora_keywords_only_for_local_scene_models() {
        ...
        assert!(has_scene_local_image_model_gate(
            remote
                .conditions
                .as_ref()
                .expect("remote gate should exist"),
            false,
        ));
