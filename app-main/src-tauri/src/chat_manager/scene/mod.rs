...
fn build_scene_generation_request(
    ...
    // Build character context for the new fields
    let character_context = Some(crate::image_generator::types::ImageCharacterContext {
        name: Some(character.name.clone()),
        description: character
            .definition
            .clone()
            .or_else(|| character.description.clone()),
        appearance: character.design_description.clone(),
        personality: None,
        reference_image: reference_images.character_images.first().cloned(),
        reference_images: reference_images.character_images.clone(),
    });

    let character_reference_image = reference_images.character_images.first().cloned();
    let character_reference_images = reference_images.character_images.clone();
    ...
    if model.provider_id == "sdcpp" {
        return ImageGenerationRequest {
            ...
            // New fields required by ImageGenerationRequest
            character_context: character_context.clone(),
            character_reference_image: character_reference_image.clone(),
            character_reference_images: character_reference_images.clone(),
            scene_context: None,
            // Output settings
