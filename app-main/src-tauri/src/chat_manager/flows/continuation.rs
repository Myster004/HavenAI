...
impl ContinueFlow {
    ...
    pub async fn execute(self, args: ChatContinueArgs) -> Result<ContinueResult, String> {
        ...
        // Context-window enforcement (preserve pinned + system)
        let mut effective_max_tokens: Option<u32> = None;
        let mut used_lorebook_entries = Vec::new();
        {
                ...
                Ok((trimmed_chat, ctx_res)) => {
                        ...
                        || ctx_res.final_output_tokens != ctx_res.requested_output_tokens
                    {
                        crate::utils::log_info(
                            ...
                            "context_window",
                            format!(
                                "continue trim model={} limit={} input={} (orig {}) requested={} final_output={} removed={}",
                                model.name,
        ...
        let assistant_message = StoredMessage {
            ...
            effective_at: if companion_mode_enabled
                && crate::chat_manager::temporal::companion_time_awareness_enabled(&session)
            {
                Some(crate::chat_manager::temporal::companion_effective_now(
                    &session,
                ))
            } else {
