...
impl CompletionFlow {
    ...
    pub async fn execute(self, args: ChatCompletionArgs) -> Result<ChatTurnResult, String> {
        ...
        // ---------------------------------------------------------------
        // Context-window enforcement (history trimming) — keeps system/
        // character/persona/memory/lore + current user, trims oldest
        // removable turns first. Preserves pinned messages.
        // ---------------------------------------------------------------
        ...
        let mut effective_max_tokens: Option<u32> = None;
        {
            // First, resolve the context length asynchronously using provider metadata
            let mut request_settings = crate::chat_manager::execution::RequestSettings::resolve(
                &session, &model, settings,
            );
            // Check if there was an explicit context_length before we override it
            let had_explicit_context = request_settings.context_length.is_some();
            let resolved_context_length = crate::chat_manager::execution::provider_fields::RequestSettings::resolve_context_length_async(
                &app,
                &session,
                &model,
                settings,
            ...
            // Create request settings with the resolved context length
            request_settings.context_length = Some(resolved_context_length);
            ...
            let pinned_count = pinned_msgs.len();
            let (trimmed_chat, ctx_res) = match crate::chat_manager::prompting::context_window::enforce_context_window_for_chat(
                &prompt_entries,
                &chat_messages,
                &system_role,
                &request_settings,
                ...
                Ok((trimmed_chat, ctx_res)) => {
                    if ctx_res.context_limit
                        == crate::chat_manager::execution::FALLBACK_CONTEXT_LENGTH
                        && !had_explicit_context
                    {
                        crate::utils::log_info(
                            &app,
                            "context_window",
                            format!(
                                "using fallback context limit {} for model={} (no explicit context_length configured)",
                                ctx_res.context_limit, model.name
                            ),
                        );
                    }
                    if ctx_res.removed_count > 0
                        || ctx_res.final_output_tokens != ctx_res.requested_output_tokens
                    {
                        crate::utils::log_info(
                            &app,
                            "context_window",
                            format!(
                                "completion trim model={} limit={} input={} (orig {}) requested={} final_output={} removed={}",
                                model.name,
                                ctx_res.context_limit,
                                ctx_res.input_tokens,
                                ctx_res.original_input_tokens,
                                ctx_res.requested_output_tokens,
                                ctx_res.final_output_tokens,
                                ctx_res.removed_count
                            ),
                        );
                        crate::utils::emit_debug(
                            ...
                            serde_json::json!({
                                ...
                                "model": model.name,
                                "providerId": credential.provider_id,
                                "contextLimit": ctx_res.context_limit,
                                "originalInputTokens": ctx_res.original_input_tokens,
                                "finalInputTokens": ctx_res.input_tokens,
                                "requestedOutput": ctx_res.requested_output_tokens,
                                "finalOutput": ctx_res.final_output_tokens,
                                "removedCount": ctx_res.removed_count,
                            }),
                        );
                    }
                    effective_max_tokens = Some(ctx_res.final_output_tokens);
                    (trimmed_chat, ctx_res)
                }
                Err(e) => return Err(e),
            };
...
let mut effective_max_tokens: Option<u32> = None;
        {
            ...
            let pinned_count = pinned_msgs.len();
            match crate::chat_manager::prompting::context_window::enforce_context_window_for_chat(
                &prompt_entries,
                &chat_messages,
                &system_role,
                &request_settings,
