...
impl RegenerateFlow {
    ...
    pub async fn execute(self, args: ChatRegenerateArgs) -> Result<RegenerateResult, String> {
        ...
        let messages_for_api = {
            ...
            // Context-window enforcement for regenerate
            let mut effective_max_tokens: Option<u32> = None;
            let mut used_lorebook_entries = Vec::new();
            {
                ...
                match crate::chat_manager::prompting::context_window::enforce_context_window_for_chat(
                    ...
                    Ok((trimmed_chat, ctx_res)) => {
                        if ctx_res.context_limit == crate::chat_manager::execution::FALLBACK_CONTEXT_LENGTH
                        ...
                        if ctx_res.removed_count > 0 || ctx_res.final_output_tokens != ctx_res.requested_output_tokens {
                            crate::utils::log_info(
                                &app,
                                "context_window",
                                format!(
                                    "regenerate trim model={} limit={} input={} (orig {}) requested={} final_output={} removed={}",
                                    model.name,
