...
fn handle_runtime_output_segment(
    ...
    let phase = if captures[3].contains("B/s") {
        "loading"
    } else {
        "sampling"
    };
...
fn hf_profile_rules(profile: &ProfileSpec) -> (Vec<&'static str>, Vec<&'static str>, f32) {
    match profile.id {
        "z-image-turbo" => (
            vec!["z-image-turbo", "z_image_turbo"],
            vec!["qwen3-4b"],
            4.0,
        ),
...
fn apply_stored_lora_keywords(lora: &mut super::types::ImageLora, info: &StoredLoraInfo) {
...
pub async fn sdcpp_discover_lora_keywords(
    ...
    let filename = file_path
        ...
        .to_string();
    let stored =
        upsert_lora_file_record(&app, &relative_path, &filename, bytes_on_disk, modified_at)?;
    ...
    if let Some(cached) = stored_lora_info_by_hash(&app, &sha256)? {
        if !has_manual_keyword_override && info.keywords.is_empty() && !cached.keywords.is_empty() {
    ...
    if (!has_manual_keyword_override && info.keywords.is_empty()) || info.architecture.is_none() {
        match reqwest::Client::new()
            ...
            .header(
                ...
                "LettuceAI LoRA metadata discovery",
            )
            .send()
            .await
        {
...
fn lora_discovery_result(info: &StoredLoraInfo, profile_id: Option<&str>) -> LoraKeywordDiscovery {
...
pub async fn sdcpp_loras(
    ...
    for file in &mut files {
        ...
        let mut info =
            upsert_lora_file_record(&app, &file.path, &file.filename, bytes_on_disk, modified_at)?;
        ...
        file.compatibility =
            lora_compatibility(file.architecture.as_deref(), profile_id.as_deref());
    }
...
pub async fn generate(
    ...
    let (width, height) = super::provider_adapter::parse_size_dimensions(
        ...
        profile
            .map(|profile| profile.default_height)
            .unwrap_or(1024),
    );
    let settings = request.advanced_model_settings.as_ref();
    let steps = settings.and_then(|settings| settings.sd_steps).unwrap_or(
        profile
            .map(|profile| profile.default_steps as u32)
            .unwrap_or(20),
    );
    let cfg = settings
        ...
        .unwrap_or(
            profile
                .map(|profile| profile.default_cfg as f64)
                .unwrap_or(7.0),
        );
...
fn upscaler_inventory(app: &AppHandle) -> Result<UpscalerInventory, String> {
    ...
    let hires_upscaler_names = models
        ...
        .collect();
    let recommended_installed = models.iter().any(|name| name == UPSCALER_MODEL_FILENAME);
...
pub async fn sdcpp_install_upscaler(app: AppHandle) -> Result<UpscalerInventory, String> {
    ...
    if !target.is_file()
        || std::fs::metadata(&target).map(|meta| meta.len()).ok() != Some(UPSCALER_MODEL_BYTES)
    {
...
pub async fn sdcpp_upscale_image(
    ...
    let work_dir = crate::utils::lettuce_dir(&app)?
        .join("cache")
        .join("upscale");
...
fn component_path(app: &AppHandle, component: ComponentSpec) -> Result<PathBuf, String> {
    ...
    let relative = Path::new("components")
...
mod tests {
    use super::{
        ...
        ensure_runtime_supports_profile, keywords_from_safetensors_metadata, lora_compatibility,
        manual_backend_specs, max_vram_spec, merge_generation_loras, migrate_legacy_compute_policy,
        oom_signature_present, prepare_sdcpp_compatible_lora, rewrite_flux_klein_lora_header,
        rewrite_flux_klein_lora_tensor_name, runtime_build_number, validate_compute_policy,
        ComputePolicy, EstimateComponent, LegacyComputePolicy, RunnabilityDevice,
        RunnabilityEstimate, RunnabilityPlacement, SdGenerationPayload, StoredLoraInfo, MIB,
        PROFILES,
    };
    ...
    fn flux_klein_lora_cache_preserves_tensor_payload() {
        ...
        let cached_header_length =
            u64::from_le_bytes(cached_bytes[..8].try_into().unwrap()) as usize;
