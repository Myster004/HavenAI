...
/// Export full app backup to a .zip file
#[tauri::command]
pub async fn backup_export(
    ...
    let filename = format!("lettuce_backup_{}.zip", timestamp);
...
pub async fn backup_import(
    ...
    if backup_path.ends_with("backup_import_temp.zip")
        || backup_path.ends_with("backup_import_temp.lettuce")
    {
    ...
    #[cfg(not(target_os = "android"))]
    {
            ...
            read_result.map_err(|e| crate::utils::err_to_string(module_path!(), line!(), e))?
        {
            ...
            if let Some(ext) = path.extension() {
                if ext == "zip" || ext == "lettuce" || ext == "haven" {
