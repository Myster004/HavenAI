...
pub struct SharedMemoryState {
    ...
    pub relationship_progress: Option<String>,
}
...
pub fn load_state(conn: &Connection, character_id: &str) -> Result<SharedMemoryState, String> {
    conn.query_row(
        "SELECT memories, memory_summary, memory_summary_token_count, memory_tool_events, memory_status, memory_error, memory_progress_step, soul_growth, relationship_states, relationship_progress
        ...
        |row| {
            Ok(SharedMemoryState {
                ...
                relationship_progress: row.get::<_, Option<String>>(9)?,
            })
        },
    )
    .optional()
...
pub fn sync_normalized_soul_facts(
    ...
    for fact in &facts {
        if existing_by_id
            .get(fact.id.as_str())
            .is_some_and(|current| *current == fact)
        {
            continue;
        }
        conn.execute(
...
fn load_episode_state(conn: &Connection, session_id: &str) -> Result<Option<JsonValue>, String> {
    if session_id.trim().is_empty() || !companion_episodes_table_exists(conn) {
        return Ok(None);
    }
    conn.query_row(
...
pub fn merge_continuity_into_state(
        ...
        .is_some_and(|items| !items.is_empty())
    {
        ...
        for fact in soul_growth.as_array().into_iter().flatten() {
            ...
            let existing_index = (!fact_id.is_empty())
                .then(|| {
...
pub fn persist_continuity_from_state(
    ...
    if let Some(soul_growth) = state_object
        .get("soulGrowth")
        .filter(|value| value.is_array())
    {
        let growth_json = serde_json::to_string(soul_growth).unwrap_or_else(|_| "[]".to_string());
        continuity.soul_growth_json = sync_normalized_soul_facts(conn, character_id, &growth_json)?;
    }
