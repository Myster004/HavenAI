 ...
 *
 * This is provider-agnostic: the caller supplies prompt/model/providerId
 * and the backend selects the correct adapter (generic_http, gradio, etc.)
 * based on the credential's providerId and baseUrl. No provider-specific
...
export async function getImageProviders(): Promise<ImageProviderInfo[]> {
  return invoke<ImageProviderInfo[]>("get_image_providers");
}

export async function testImageProviderConnection(credentialId: string): Promise<string> {
  return invoke<string>("test_image_provider_connection", { credentialId });
}

export async function getImageProviderModels(credentialId: string): Promise<string[]> {
  return invoke<string[]>("get_image_provider_models", { credentialId });
}
...
export function getModelSizes(providerId: string, modelId: string): readonly string[] {
  ...
  if (providerId === "stability" || providerId === "aihorde") {
    return ["512x512", "768x768", "1024x1024", "1152x896", "896x1152"];
  }
