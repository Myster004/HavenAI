...
export function ProvidersPage() {
  ...
  const isLocalProvider =
    !!editorProvider &&
    ["ollama", "lmstudio", "intenserp", "automatic1111", "comfyui", "diffusers", "aihorde"].includes(
      editorProvider.providerId,
    );
  const isComfyUiProvider = !!editorProvider && editorProvider.providerId === "comfyui";
  const isGenericImageProvider =
    !!editorProvider && ["generic_http", "gradio"].includes(editorProvider.providerId);
  const isGradioProvider = !!editorProvider && editorProvider.providerId === "gradio";
  const isGenericHttpProvider = !!editorProvider && editorProvider.providerId === "generic_http";
  const isCustomProvider =
    !!editorProvider &&
    (editorProvider.providerId === "custom" || editorProvider.providerId === "custom-anthropic");
  const allowsTlsException = !!editorProvider && (isLocalProvider || isCustomProvider || isGenericImageProvider);
  const showBaseUrl =
    !!editorProvider &&
    (isLocalProvider || isCustomProvider || isEngineProvider || isHostProvider || isGenericImageProvider);
  ...
  const selectedCapability = editorProvider
    ...
    : null;
  const genericAuthType = ((customConfig.authType as string | undefined) ?? "none").toLowerCase();
  const providerRequiresApiKey = isGenericImageProvider
          ...
          >
            {editorProvider && (
              <div className="space-y-4 pb-2">
                ...
                {isGenericImageProvider && (
                  <div className="space-y-3 rounded-lg border border-fg/10 bg-surface/20 p-3">
                    <h4 className="text-sm font-medium text-fg">Image Provider Configuration</h4>
                    <p className="text-xs leading-relaxed text-fg/50">
                      Configure any HTTP image generation endpoint. For Hugging Face Docker Spaces, use Generic HTTP with endpoint like https://your-space.hf.space/generate. For Gradio Spaces, use Gradio provider with Space URL as Base URL.
                    </p>
                    {isGenericHttpProvider && (
                      <>
                        <TextField
                          label="Endpoint Path (optional, appended to Base URL)"
                          value={(customConfig.endpointPath as string | undefined) ?? ""}
                          onChange={(value) =>
                            updateEditorProvider({
                              config: { ...editorProvider.config, endpointPath: value || undefined },
                            })
                          }
                          placeholder="/generate or /api/generate"
                        />
                        <SelectField
                          label="Authentication Type"
                          value={(customConfig.authType as string | undefined) ?? "none"}
                          onChange={(value) =>
                            updateEditorProvider({
                              config: { ...editorProvider.config, authType: value },
                            })
                          }
                        >
                          ...
                          <option value="apiKey" className="bg-surface-el">
                            API Key (x-api-key)
                          </option>
                        ...
                        {genericAuthType !== "none" && (
                          <TextField
                            label="Auth Header Name (for API Key)"
                            value={(customConfig.authHeaderName as string | undefined) ?? "x-api-key"}
                            onChange={(value) =>
                              updateEditorProvider({
                                config: { ...editorProvider.config, authHeaderName: value || undefined },
                              })
                            }
                        ...
                        <TextField
                          label="Request Template (optional JSON with {{prompt}}, {{width}}, etc.)"
                          value={(customConfig.requestTemplate as string | undefined) ?? ""}
                          onChange={(value) =>
                            updateEditorProvider({
                              config: { ...editorProvider.config, requestTemplate: value || undefined },
                            })
                          }
                          placeholder='{"prompt": "{{prompt}}", "width": {{width}}}'
                        />
                        <TextField
                          label="Response Image Field (JSON path)"
                          value={(customConfig.responseImageField as string | undefined) ?? ""}
                          onChange={(value) =>
                            updateEditorProvider({
                              config: { ...editorProvider.config, responseImageField: value || undefined },
                            })
                          }
                          placeholder="image or data[0] or images"
                        />
                        <SelectField
                          label="Response Format"
                          value={(customConfig.responseFormat as string | undefined) ?? "json"}
                          onChange={(value) =>
                            updateEditorProvider({
                              config: { ...editorProvider.config, responseFormat: value },
                            })
                          }
                        >
                          <option value="json" className="bg-surface-el">
                            JSON (base64 or URL)
                          </option>
                          <option value="binary" className="bg-surface-el">
                            Binary Image
                          </option>
                        </SelectField>
                      </>
                    ...
                    {isGradioProvider && (
                      <>
                        <TextField
                          label="Gradio Endpoint Name"
                          value={(customConfig.gradioEndpoint as string | undefined) ?? "predict"}
                          onChange={(value) =>
                            updateEditorProvider({
                              config: { ...editorProvider.config, gradioEndpoint: value || undefined },
                            })
                          }
                          placeholder="predict"
                        />
                        <TextField
                          label="Gradio Function Index"
                          value={String((customConfig.gradioFnIndex as number | undefined) ?? 0)}
                          onChange={(value) =>
                            updateEditorProvider({
                              config: { ...editorProvider.config, gradioFnIndex: parseInt(value) || 0 },
                            })
                          }
                          placeholder="0"
                        />
                        <SelectField
                          label="Use Queue (Gradio 4+)"
                          value={String((customConfig.gradioUseQueue as boolean | undefined) ?? true)}
                          onChange={(value) =>
                            updateEditorProvider({
                              config: { ...editorProvider.config, gradioUseQueue: value === "true" },
                            })
                          }
                        >
                          <option value="true" className="bg-surface-el">
                            Yes (queue)
                          </option>
                          <option value="false" className="bg-surface-el">
                            No (direct /api/predict)
                          </option>
                        </SelectField>
                        <SelectField
                          label="Authentication Type"
                    ...
                    <TextField
                      ...
                      value={String((customConfig.timeoutSeconds as number | undefined) ?? 120)}
                      onChange={(value) =>
                        updateEditorProvider({
                          config: { ...editorProvider.config, timeoutSeconds: parseInt(value) || 120 },
                        })
                      }
                      placeholder="120"
                    />
                    <div className="flex gap-2">
                      <button
                        ...
                        onClick={async () => {
                          try {
                            const result = await invoke<string>("test_image_provider_connection", {
                        ...
                        className="flex-1 rounded-lg border border-accent/30 bg-accent/10 px-3 py-2 text-sm font-medium text-accent hover:bg-accent/20"
                      >
                        Test Connection
                      </button>
                      <button
                        ...
                        onClick={async () => {
                          try {
                            const models = await invoke<string[]>("get_image_provider_models", {
                              credentialId: editorProvider.id,
                            });
                            alert(`Found ${models.length} models: ${models.slice(0, 5).join(", ")}${models.length > 5 ? "..." : ""}`);
                          } catch (e) {
                            alert(`Failed to fetch models: ${String(e)}`);
                          }
                        }}
                        className="flex-1 rounded-lg border border-fg/10 bg-fg/5 px-3 py-2 text-sm font-medium text-fg hover:bg-fg/10"
                      >
                        Refresh Models
                      </button>
                    </div>
                    <p className="text-[11px] leading-relaxed text-fg/40">
                      Model is independent from provider. Set Base URL to your hosting (localhost, HF Space, VPS, etc.), set Model to DreamShaper_8, Flux, SDXL, or any model your endpoint supports. No source changes needed.
                    </p>
                  </div>
                )}
