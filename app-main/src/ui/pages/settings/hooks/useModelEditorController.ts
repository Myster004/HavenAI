...
function isImageOnlyProvider(providerId?: string | null): boolean {
  return (
    ...
    providerId === "gradio" ||
    providerId === "comfyui" ||
    providerId === "diffusers" ||
    providerId === "openai" ||
    providerId === "stability" ||
    providerId === "pollinations" ||
    providerId === "xai" ||
    providerId === "nanogpt" ||
    providerId === "literouter" ||
    providerId === "gemini"
  );
}
...
const fetchModels = useCallback(async () => {
  ...
  // Local / non-remote providers – no remote model list
  if (
  ...
  if (
    (providerCred.providerId === "custom" ||
      providerCred.providerId === "custom-anthropic") &&
  ...
  try {
    // AI Horde – use dedicated command
    if (editorModel.providerId === "aihorde") {
      const names = await invoke<string[]>("get_aihorde_model_names", {
        apiKey: providerCred.apiKey || "0000000000",
      });

      // Convert string names into the shape the UI expects
    ...
    // All other providers – existing remote models endpoint
