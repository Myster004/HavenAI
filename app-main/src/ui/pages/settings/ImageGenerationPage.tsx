      ...
      <main className="flex-1 px-4 pb-24 pt-5">
        <div className={cn("mx-auto w-full max-w-5xl space-y-7", spacing.section)}>
          ...
          <div className="space-y-7">
            ...
            <SettingsSection
              title="Custom Image Providers (Provider-Agnostic)"
              icon={<Image size={12} />}
            >
              <div className="rounded-[12px] border border-fg/10 bg-fg/5 p-4">
                <p className="text-sm leading-6 text-fg/60">
                  Haven can use any HTTP image generation endpoint without source changes. Create a provider via{" "}
                  <span className="font-medium text-fg">Settings → Providers</span> with type{" "}
                  <span className="font-medium text-fg">Generic HTTP</span> or{" "}
                  <span className="font-medium text-fg">Gradio</span>, set Base URL to your endpoint
                  (e.g. <span className="font-mono text-xs">https://your-space.hf.space</span>,{" "}
                  <span className="font-mono text-xs">http://localhost:7860</span>,{" "}
                  <span className="font-mono text-xs">https://myserver.example.com/generate</span>), and create a model with
                  provider Generic HTTP and model name <span className="font-medium">DreamShaper_8</span>,{" "}
                  <span className="font-medium">Flux</span>, <span className="font-medium">SDXL</span>, etc. The frontend calls{" "}
                  <span className="font-mono text-xs">generateImage(prompt, options)</span> and the backend adapter normalizes
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  <button
                    ...
                    className="rounded-full border border-accent/30 bg-accent/10 px-4 py-1.5 text-xs font-medium text-accent hover:bg-accent/20"
                  >
                    Go to Providers
                  </button>
                  <button
                    type="button"
                    onClick={() => navigate("/settings/models")}
                    className="rounded-full border border-fg/10 bg-fg/5 px-4 py-1.5 text-xs font-medium text-fg/70 hover:bg-fg/10"
                  >
                    Manage Models
                  </button>
                </div>
                <div className="mt-4 grid grid-cols-1 gap-2 text-xs leading-relaxed text-fg/45">
                  <div>• Generic HTTP: any REST JSON or binary image API (DreamShaper 8, SDXL, Flux on custom server, Hugging Face Docker Spaces at https://my-image-generator.hf.space/generate)</div>
                  <div>• Gradio: Hugging Face Spaces with Gradio UI (https://username-dreamshaper-space.hf.space, predict endpoint). Handles Gradio queue (/gradio_api/call/predict) and direct /api/predict.</div>
                  <div>• Hosting agnostic: localhost, LAN IP, HTTPS, HF Space, VPS, Cloudflare, Render, Railway, Fly.io, custom FastAPI server</div>
                  <div>• Model independent: change Model field from DreamShaper_8 to Flux/SDXL without code. Auth via provider credential (None/Bearer/API Key/Custom Header, env IMAGE_API_KEY).</div>
                  <div>• Response normalization: Handles {"{image: base64}"}, {"{url: https://...}"}, {"{images: [...] }"}, {"{data: [...] }"}, binary image, and remote URL download.</div>
                </div>
              </div>
            </SettingsSection>
          </div>
